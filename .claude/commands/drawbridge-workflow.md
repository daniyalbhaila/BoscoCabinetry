...
**File Locations**: Task files are always located in the `.moat/` directory at the project root:
- **JSON Data**: `.moat/moat-tasks-detail.json` (primary source of truth)
- **Markdown Checklist**: `.moat/moat-tasks.md` (human-readable task list)
...
1.  **Locate Screenshot**: Use the `screenshotPath` from the JSON data (typically `.moat/screenshots/moat-[timestamp]-[id].png`)
...
-   `.moat/moat-tasks.md`: Mark tasks as complete (`[x]`) once their status is `done`.

-   `.moat/moat-tasks-detail.json`: Update the task `status` through its lifecycle with proper validation.
