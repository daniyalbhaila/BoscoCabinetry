...
function main() {
  ...
  Object.entries(byType).forEach(([type, data]) => {
    console.log(
      `${type.padEnd(10)} | ${data.files.length.toString().padStart(5)} files | ${formatBytes(data.totalSize).padStart(10)}`,
    );
  });
  ...
  analysis.files.slice(0, 10).forEach((file) => {
    console.log(
      `${file.name.padEnd(45)} | ${formatBytes(file.size).padStart(10)}`,
    );
  });
  ...
  const largeJS = analysis.files.filter(
    (f) => f.type === "js" && f.size > 100000,
  );
  if (largeJS.length > 0) {
  ...
  const images = analysis.files.filter((f) =>
    ["jpg", "jpeg", "png", "webp", "gif"].includes(f.type),
  );
  if (images.length > 0) {
    const totalImageSize = images.reduce((sum, img) => sum + img.size, 0);
    console.log(
      `📸 Images: ${images.length} files, ${formatBytes(totalImageSize)} total`,
    );
  }
